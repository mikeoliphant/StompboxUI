...
sustain		= nentry("sustain", 20, 0, 40, 0.1);
threshold = -sustain;
knee		= nentry("knee", 3, 0, 20, 0.1);

// attack and release controls; clamped to a minimum of 1 sample
attack		= hslider("attack", 0.02, .002, 0.05, 0.001) : max(1/ma.SR);
release		= hslider("release", 0.2, 0, 10, 0.01) : max(1/ma.SR);
...
makeup = (-threshold * (1 - (1 / ratio))) / 2 : ba.db2linear;
...
wet = vslider("wet_dry[name:wet/dry][tooltip:percentage of processed signal in output signal]",  1, 0, 1, .01);
dry = 1 - wet;

process = _<:*(dry),(*(wet): comp):>_;
