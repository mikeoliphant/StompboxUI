...
t = vslider(".amp.tonestack.Treble[alias]", 5, 0, 10, 0.1) / 10;
m = vslider(".amp.tonestack.Middle[alias]", 5, 0, 10, 0.1) / 10;
l = vslider(".amp.tonestack.Bass[alias]", 5, 0, 10, 0.1) / 10; // : (_-1)*3.4 : exp;

tonestack = 1/A0*fi.iir((B0,B1,B2,B3),(A1/A0,A2/A0,A3/A0)) with {
    C1 = fconstant(float C1, <math.h>);
    C2 = fconstant(float C2, <math.h>);
    C3 = fconstant(float C3, <math.h>);
    R1 = fconstant(float R1, <math.h>);
    R2 = fconstant(float R2, <math.h>);
    R3 = fconstant(float R3, <math.h>);
    R4 = fconstant(float R4, <math.h>);
